...
describe Yaks::Mapper::Association do
  describe 'self_link' do
    context 'without a :self link' do
      subject { described_class.new(nil, nil, Yaks::List(Yaks::Mapper::Link.new(:alternate, '/foo'))) }
  ...
  context 'with a self link' do
    subject { described_class.new(nil, nil, Yaks::List(Yaks::Mapper::Link.new(:self, '/self'))) }
